...
function VerificationBanner({ token }) {
  ...
  return (
    <div className="bg-amber-500/10 border-b border-amber-500/20 py-2.5 px-4 text-center text-xs text-amber-400 flex items-center justify-center gap-2 flex-wrap">
      <AlertTriangle className="h-4 w-4 shrink-0" />
      <span>
        Your email address is unverified. Please check your email inbox (or spam folder) to retrieve the verification link.
      </span>
      {message ? (
        <span className="text-emerald-400 font-bold ml-2">{message}</span>
      ) : error ? (
        <span className="text-red-400 font-bold ml-2">{error}</span>
      ) : (
        <button
          onClick={handleResend}
          disabled={loading}
          className="ml-2 px-2.5 py-0.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 font-bold border border-amber-500/30 rounded-md transition-all cursor-pointer disabled:opacity-50"
        >
